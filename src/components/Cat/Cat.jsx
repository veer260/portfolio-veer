...
function Cat() {
    ...
    function handleTouchEnd(e) {
        e.preventDefault();
        setIsTouching(false);
        // If it was a tap on the wrapper (not canvas), move the cat
    ...
    // Calculate eye shifts based on mouse OR touch position
    React.useEffect(() => {
        if(canvasRef.current) {
            const startX = canvasRef.current.getBoundingClientRect().left;
            const startY = canvasRef.current.getBoundingClientRect().top;
            ...
            const currentInputPos = isTouching ? touchPos : mousePos;
            const y = currentInputPos.y - centerY;
            const x = currentInputPos.x - centerX;
            const rad = Math.atan2(y, x);   
            let deg = rad * 180 / Math.PI;
            if(deg < 0){
                deg = deg*-1;
    ...
    return (
        ...
        >
            <canvas 
                onMouseLeave={() => setIsBeingPet(false)} 
                onMouseDown={(e) => setIsBeingPet(true)}
                onMouseUp={() => setIsBeingPet(false)}
